...
function createKin(data) {
  ...
  const forecastHour = data.forecastHour != null ? Number(data.forecastHour) : null;

  getDb().prepare(`
    INSERT INTO kins (id, name, kindroid_key, ai_id, latitude, longitude,
...
function updateKin(id, data) {
  ...
  if (data.kindroidKey !== undefined) fields.kindroid_key = data.kindroidKey;
